...
function getPolynom(...args) {
  ...
  return (x) => {
    ...
    args.forEach((item, index) => {
      result += item * x ** (args.length - index - 1);
    });
    return result;
  };
}
...
function memoize(func) {
  let cache;
  let wasICalledBefore = false;

  return () => {
    if (!wasICalledBefore) {
      wasICalledBefore = true;
      cache = func();
    }
    return cache;
  };
...
function logger(func, logFunc) {
  return (...args) => {
    ...
    const startLog = `${func.name}(${argsToString}) starts`;
    const endLog = `${func.name}(${argsToString}) ends`;
    logFunc(startLog);
